...
from geo_attn.modules import Geo_Attention_Model, Geo_Attention_Model2, Geo_Attention_Model3
...
def train2(args):
    ...
    model = Geo_Attention_Model2().to(args.device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    criterion = torch.nn.MSELoss()
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader):
            ...
            locs, readings, target_loc, target_reading = locs.to(args.device), readings.to(args.device), target_loc.to(args.device), target_reading.to(args.device)
            optimizer.zero_grad()
            pred = model((locs, readings, target_loc, target_reading))
            loss = criterion(pred, target_reading[:, -2:-1])
            loss.backward()
            train_loss += loss.cpu().detach().item()
            optimizer.step()
        train_loss /= len(train_loader)
...
def train3(args):
    ...
    model = Geo_Attention_Model3().to(args.device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    criterion = torch.nn.MSELoss()
